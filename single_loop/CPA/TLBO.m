function [Elite_antlion_fitness,teacher,Convergence_curve,G]=TLBO(ant_position,Np,Gm,Lowerbound,Upperbound,D,func_num)

func_init=PID_function(ant_position(1,1),ant_position(1,2),ant_position(1,3),func_num,1);
...
ite_flag=9;
while abs(Elite_antlion_fitness-ite_flag)>10E-7
    %***************Teacher Phase**************%
    for i=1:D
        Sum_Column=0;
        for j=1:Np
    ...
    for j=1:Np
        Fit_Tr(j)=PID_function(St(j,1),St(j,2),St(j,3),func_num,0);
    end
    [Y,Tridex]=min(Fit_Tr);
    Tr(G,:)=St(Tridex,:);
    for k=1:Np
        ...
        if PID_function(St_new(k,1),St_new(k,2),St_new(k,3),func_num,0)<PID_function(St(k,1),St(k,2),St(k,3),func_num,0)
            St(k,:)=St_new(k,:);
        end
    end
    ...
    for i=1:Np
        dx=randperm(Np); 
        if dx(1)==i 
            if PID_function(St(i,1),St(i,2),St(i,3),func_num,0)<PID_function(St(dx(2),1),St(dx(2),2),St(dx(2),3),func_num,0)
                St_new(i,:)=St(i,:)+rand*(St(i,:)-St(dx(2),:));
            else
                St_new(i,:)=St(i,:)+rand*(St(dx(2),:)-St(i,:));
            end
        else
            if PID_function(St(i,1),St(i,2),St(i,3),func_num,0)<PID_function(St(dx(1),1),St(dx(1),2),St(dx(1),3),func_num,0)
        ...
        if PID_function(St_new(i,1),St_new(i,2),St_new(i,3),func_num,0)<PID_function(St(i,1),St(i,2),St(i,3),func_num,0)
            St(i,:)=St_new(i,:);
        end
    end
    ...
    if G>22
        ite_flag=Convergence_curve(G-22);
    end
